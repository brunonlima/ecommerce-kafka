...
public class BatchSendMessageService {
    ...
    private void parse(ConsumerRecord<String, Message<String>> record) throws SQLException, ExecutionException, InterruptedException {
        ...
        //Force error, to consume DeadLetter topic
        //if (true) throw new RuntimeException("processing topic error!");

        for(User user : getAllUsers()) {
            userDispatcher.sendAsync(message.getPayload(),user.getUuid(),
                    message.getId().continueWithCorrelationId(BatchSendMessageService.class.getSimpleName())
                    ,user);
